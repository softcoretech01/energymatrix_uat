...
function EnergyAllotment() {
    ...
    const handleViewAllotmentDetails = async (wmNumber: string) => {
        const windmillObj = windmillsDetailed.find(w => String(w.windmill_number || '').trim() === String(wmNumber).trim());
        if (!windmillObj) {
            toast.error(`Windmill details not found for ${wmNumber}`);
            return;
        }

        setIsLoadingDetails(true);
        setSelectedAllotmentDetails(null);
        setIsDetailsModalOpen(true);

        try {
            const response = await api.get(`/windmills/allotment-order/details`, {
                params: {
                    windmill_id: windmillObj.id,
                    year: selectedYear,
                    month: selectedMonth
                }
            });
            ...
            if (response.data && response.data.status === "success" && response.data.data) {
                setSelectedAllotmentDetails({
                    windmill_number: wmNumber,
                    year: selectedYear,
                    month: months.find(m => m.value === selectedMonth)?.label || selectedMonth,
                    ...response.data.data
                });
            } else {
                toast.error("No allotment details found for this selection.");
                setIsDetailsModalOpen(false);
            }
        } catch (error) {
            console.error("Error fetching allotment details:", error);
            toast.error("Failed to load allotment order details.");
            setIsDetailsModalOpen(false);
        } finally {
    ...
    const handleFileUpload = async (wm: string, file: File | null) => {
        ...
        try {
            ...
            const errMsg = error.response?.data?.message || error.response?.data?.detail || "Failed to upload file.";
            toast.error(errMsg);
        }
    };
        ...
        <ErrorBoundary>
                                                    ...
                                                    {windmillNumbers.map((wm) => {
                                                        return (
                                                            <TableHead key={wm} colSpan={4} className="font-semibold text-center border-b border-r border-white/20 p-0 bg-sidebar sticky top-0 z-40 h-[50px]">
                                                                <div className="text-white h-full flex items-center justify-center gap-1.5">
                                                                    <span>{wm}</span>
                                                                    {uploads[wm]?.fileName && (
                                                                        <button
                                                                            type="button"
                                                                            className="text-white/80 hover:text-white hover:bg-white/10 p-0.5 rounded transition-colors"
                                                                            onClick={() => handleViewAllotmentDetails(wm)}
                                                                            title="View Allotment Order Details"
                                                                        >
                                        ...
                                        <Table>
                                            <TableHeader className="bg-sidebar">
                                                <TableRow>
                                                    ...
                                                    <TableHead className="h-8 py-1 font-semibold text-white whitespace-nowrap text-xs">Upload Files</TableHead>
                                                    <TableHead className="h-8 py-1 font-semibold text-white whitespace-nowrap text-xs text-center w-24">View</TableHead>
                                                </TableRow>
                                            </TableHeader>
                                            <TableBody>
                                                        ...
                                                        <TableCell className="py-2 text-center">
                                                            {uploads[wm]?.fileName ? (
                                                                <Button
                                                                    variant="ghost"
                                                                    size="icon"
                                                                    className="h-6 w-6 text-indigo-600 hover:text-indigo-800 hover:bg-indigo-50 rounded-full mx-auto"
                                                                    onClick={() => handleViewAllotmentDetails(wm)}
                                                                    title="View Allotment Order Details"
                                                                >
                                                                    <Eye className="h-3.5 w-3.5" />
            ...
            {isDetailsModalOpen && (
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] p-4 backdrop-blur-sm transition-all duration-300">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col border border-slate-200 animate-in fade-in zoom-in-95 duration-200">
                        {/* Modal Header */}
                        <div className="p-4 bg-gradient-to-r from-indigo-700 to-indigo-900 text-white flex justify-between items-center shadow-md">
                            <div className="flex flex-col">
                                <h2 className="text-lg font-bold flex items-center gap-2">
                                    Allotment Order Data Stored
                                </h2>
                                <p className="text-xs text-indigo-100 mt-0.5">
                                    Windmill: <span className="font-bold text-white bg-indigo-800 px-2 py-0.5 rounded ml-1">{selectedAllotmentDetails?.windmill_number}</span>
                                    {" • "} Period: <span className="font-bold text-white">{selectedAllotmentDetails?.month} {selectedAllotmentDetails?.year}</span>
                                </p>
                            </div>
                            <Button 
                                variant="ghost" 
                                className="text-white hover:text-slate-200 hover:bg-white/10 h-8 w-8 p-0 rounded-full font-bold" 
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
                                        <div className="p-3 bg-slate-100 border-b border-slate-200">
                                            <h3 className="font-bold text-slate-700 text-sm">Parsed Slot Details</h3>
                                        </div>
                                        <div className="overflow-x-auto">
                                            <table className="min-w-full divide-y divide-slate-200 text-sm">
                                                <thead className="bg-slate-50">
                                                    <tr>
                                                        <th className="px-4 py-3 text-left font-bold text-slate-700 uppercase tracking-wider">Slot</th>
                                                        <th className="px-4 py-3 text-right font-bold text-slate-700 uppercase tracking-wider">From Powerplant</th>
                                                        <th className="px-4 py-3 text-right font-bold text-slate-700 uppercase tracking-wider">From Banking</th>
                                                        <th className="px-4 py-3 text-right font-bold text-indigo-700 uppercase tracking-wider bg-indigo-50/50">Total</th>
                                                        <th className="px-4 py-3 text-right font-bold text-slate-700 uppercase tracking-wider">Balance Banking</th>
                                                        <th className="px-4 py-3 text-right font-bold text-slate-700 uppercase tracking-wider">Balance Lapsed</th>
                                                        <th className="px-4 py-3 text-right font-bold text-slate-700 uppercase tracking-wider">Balance Surplus</th>
                                                    </tr>
                                                </thead>
                                                <tbody className="bg-white divide-y divide-slate-200">
                                                    {selectedAllotmentDetails.details && selectedAllotmentDetails.details.length > 0 ? (
                                                        selectedAllotmentDetails.details.map((detail: any) => (
                                                            <tr key={detail.id} className="hover:bg-slate-50">
                                                                <td className="px-4 py-2.5 font-bold text-slate-700">C{detail.slots}</td>
                                                                <td className="px-4 py-2.5 text-right font-medium text-slate-600">{formatWithCommas(detail.from_powerplant)}</td>
                                                                <td className="px-4 py-2.5 text-right font-medium text-slate-600">{formatWithCommas(detail.from_banking)}</td>
                                                                <td className="px-4 py-2.5 text-right font-bold text-indigo-700 bg-indigo-50/30">{formatWithCommas(detail.total)}</td>
                                                                <td className="px-4 py-2.5 text-right font-medium text-slate-600">{formatWithCommas(detail.balance_banking)}</td>
                                                                <td className="px-4 py-2.5 text-right font-medium text-slate-600">{formatWithCommas(detail.balance_lapsed)}</td>
                                                                <td className="px-4 py-2.5 text-right font-medium text-slate-600">{formatWithCommas(detail.balance_surplus)}</td>
                                                            </tr>
                                                        ))
                                                    ) : (
                                                        <tr>
                                                            <td colSpan={7} className="px-4 py-8 text-center text-slate-500 italic">
                                                                No slot records found.
                                                            </td>
                                                        </tr>
                                                    )}
                                                </tbody>
                                                {selectedAllotmentDetails.details && selectedAllotmentDetails.details.length > 0 && (
                                                    <tfoot className="bg-slate-100 font-bold border-t-2 border-slate-200">
                                                        <tr>
                                                            ...
                                                            <td className="px-4 py-3 text-right text-slate-800">{formatWithCommas(selectedAllotmentDetails.details.reduce((sum: number, d: any) => sum + Number(d.from_powerplant || 0), 0))}</td>
                                                            <td className="px-4 py-3 text-right text-slate-800">{formatWithCommas(selectedAllotmentDetails.details.reduce((sum: number, d: any) => sum + Number(d.from_banking || 0), 0))}</td>
                                                            <td className="px-4 py-3 text-right text-indigo-700 bg-indigo-50/50">{formatWithCommas(selectedAllotmentDetails.details.reduce((sum: number, d: any) => sum + Number(d.total || 0), 0))}</td>
                                                            <td className="px-4 py-3 text-right text-slate-800">{formatWithCommas(selectedAllotmentDetails.details.reduce((sum: number, d: any) => sum + Number(d.balance_banking || 0), 0))}</td>
                                                            <td className="px-4 py-3 text-right text-slate-800">{formatWithCommas(selectedAllotmentDetails.details.reduce((sum: number, d: any) => sum + Number(d.balance_lapsed || 0), 0))}</td>
                                                            <td className="px-4 py-3 text-right text-slate-800">{formatWithCommas(selectedAllotmentDetails.details.reduce((sum: number, d: any) => sum + Number(d.balance_surplus || 0), 0))}</td>
                                                        </tr>
                                                    </tfoot>
                                                )}
                                            </table>
                                    ...
                                    <div className="bg-indigo-50/50 border border-indigo-100 rounded-lg p-4 text-xs text-indigo-800 space-y-1">
                                        <p className="font-semibold text-[13px] text-indigo-900 mb-1">Document Information</p>
                                        <p>File Name: <span className="font-bold">{selectedAllotmentDetails.header?.file_name}</span></p>
                                        <p>Database File Path: <span className="font-mono">{selectedAllotmentDetails.header?.file_path}</span></p>
                                        <p>Uploaded At: <span>{selectedAllotmentDetails.header?.created_at ? new Date(selectedAllotmentDetails.header.created_at).toLocaleString() : 'N/A'}</span></p>
                                    </div>
                                </div>
                            ) : (
                                <div className="text-center text-slate-500 py-8 italic">No data to display.</div>
                            )}
                        </div>
