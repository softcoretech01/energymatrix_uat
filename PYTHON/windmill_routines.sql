...
BEGIN
                SELECT 
                    ...
                    IFNULL(mw.windmill_name, CONCAT('Windmill ', a.energy_number)) AS windmill_name,
                    a.calculated_wheeling_value AS wheeling_charges,
                    a.updated_windmill_unit,
                    a.id as actual_id,
                    t.total_self_gen_tax AS self_gen_tax,
                    t.total_wheeling,
                    t.total_updated_units
                ...
                CROSS JOIN (
                    SELECT 
                        ...
                        IFNULL(SUM(IFNULL(updated_windmill_unit, calculated_wheeling_value)), 0) AS total_updated_units,
                        IFNULL(SUM(IFNULL(updated_windmill_unit, calculated_wheeling_value)), 0) * (SELECT IFNULL(cost, 0.1) FROM masters.master_consumption_chargers WHERE charge_code = 'C011' LIMIT 1) AS total_self_gen_tax
                ...
                LEFT JOIN masters.master_windmill mw
                    ON TRIM(a.energy_number) = TRIM(mw.windmill_number) COLLATE utf8mb4_unicode_ci
...
BEGIN
            DECLARE v_tax_rate DECIMAL(10,4) DEFAULT 0.1;
            
            SELECT IFNULL(cost, 0.1) INTO v_tax_rate 
            FROM masters.master_consumption_chargers 
            WHERE charge_code = 'C011' LIMIT 1;

            SELECT (
                (SELECT IF(SUM(IFNULL(updated_windmill_unit, 0)) = 0, 
                           SUM(IFNULL(calculated_wheeling_value, 0) * v_tax_rate), 
                           SUM(IFNULL(updated_windmill_unit, 0) * v_tax_rate))
                 FROM windmill.actual 
                 WHERE customer_id = p_customer_id AND sc_id = p_sc_id AND actual_year = p_year AND actual_month = p_month) +
                (SELECT IFNULL(SUM(cad.charge_amount), 0) 
                 FROM windmill.charge_allotment_header cah 
                 JOIN windmill.charge_allotment_details cad ON cah.id = cad.header_id 
                 JOIN masters.master_consumption_chargers mcc ON cad.charge_id = mcc.id 
                 WHERE cah.customer_id = p_customer_id AND cah.service_id = p_sc_id AND cah.year = p_year AND cah.month = p_month AND mcc.charge_code = 'C011') +
                (SELECT IFNULL(SUM(scad.allocation), 0) 
                 FROM windmill.solar_charge_allotment_header scah 
                 JOIN windmill.solar_charge_allotment_details scad ON scah.id = scad.header_id 
                 JOIN masters.master_consumption_chargers mcc ON scad.charge_id = mcc.id 
                 WHERE scah.customer_id = p_customer_id AND scah.service_id = p_sc_id AND scah.year = p_year AND scah.month = p_month AND mcc.charge_code = 'C011')
            ) as total_tax;
        END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_get_actual_units_total` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `sp_get_actual_units_total`(
            IN p_customer_id INT,
            IN p_sc_id INT,
            IN p_year INT,
            IN p_month INT
...
BEGIN
            SELECT 
                IF(SUM(IFNULL(updated_windmill_unit, 0)) = 0, 
                   SUM(IFNULL(calculated_wheeling_value, 0)), 
                   SUM(IFNULL(updated_windmill_unit, 0))) as total_units 
            FROM windmill.actual 
            WHERE customer_id = p_customer_id AND sc_id = p_sc_id AND actual_year = p_year AND actual_month = p_month;
        END ;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `sp_get_client_invoice_by_id`(IN p_id INT)
BEGIN
            ...
            SELECT ci.service_id, ci.year, ci.month, ci.customer_id, mc.customer_name
            INTO v_service_id, v_year, v_month_name, v_cust_id, v_cust_name
            ...
            -- 1. Generated Units with fallback
            SELECT 
                IF(SUM(IFNULL(updated_windmill_unit, 0)) = 0, 
                   SUM(IFNULL(calculated_wheeling_value, 0)), 
                   SUM(IFNULL(updated_windmill_unit, 0)))
            ...
            FROM masters.master_customers mc
            WHERE mc.id = v_cust_id;

            -- 3. Base Charges from Allotment
            SELECT 
            ...
            INTO v_charge_meter, v_charge_om, v_charge_trans, v_charge_sys_opr, v_charge_rkvah,
            ...
            -- 4. Update Wheeling from EB bill
            ...
            -- 5. Update Tax based on Generated Units
            IF v_cust_name NOT LIKE '%L&T%' THEN
                SET v_charge_tax = v_generated_units * (SELECT IFNULL(cost, 0) FROM masters.master_consumption_chargers WHERE charge_code = 'C011' LIMIT 1);
            ELSE
                SET v_charge_tax = 0;
            END IF;
...
BEGIN
            ...
            SELECT 
                ...
                (SELECT IF(SUM(IFNULL(updated_windmill_unit, 0)) = 0, SUM(IFNULL(calculated_wheeling_value, 0)), SUM(IFNULL(updated_windmill_unit, 0)))
                 FROM windmill.actual
                ...
                IF(v_cust_name LIKE '%L&T%', 0, 
                    (SELECT IF(SUM(IFNULL(updated_windmill_unit, 0)) = 0, SUM(IFNULL(calculated_wheeling_value, 0)), SUM(IFNULL(updated_windmill_unit, 0))) 
                     FROM windmill.actual 
                     WHERE customer_id = v_cust_id AND sc_id = v_service_id AND actual_year = v_year AND actual_month = v_month_int) *
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_get_eb_wheeling_total` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`%` PROCEDURE `sp_get_eb_wheeling_total`(
                    IN p_customer_id INT,
                    IN p_sc_id INT,
                    IN p_year INT,
                    IN p_month INT
...
BEGIN
                    -- Calculate wheeling ONLY from eb_bill_adjustment_charges as requested
                    SELECT IFNULL(SUM(ac.wheeling_charges), 0) as total_wheeling
                    FROM windmill.eb_bill_adjustment_charges ac
                    JOIN windmill.eb_bill h ON ac.eb_bill_header_id = h.id
                    WHERE h.customer_id = p_customer_id 
                      AND h.sc_id = p_sc_id 
                      AND h.bill_year = p_year 
                      AND h.bill_month = p_month;
                END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
BEGIN
                ...
                FROM windmill.actual a
                LEFT JOIN masters.master_windmill mw
                  ON a.energy_number = mw.windmill_number COLLATE utf8mb4_unicode_ci
                LEFT JOIN windmill.eb_bill_adjustment_charges ebac 
                  ON a.client_eb_id = ebac.eb_bill_header_id 
                 AND a.energy_number = ebac.energy_number COLLATE utf8mb4_unicode_ci
                WHERE a.client_eb_id = p_client_eb_id;
            END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
-- Dump completed on 2026-05-12 12:06:32
